...
function getEQ4(m: CvssMetrics): number {
  const SI = m.MSI || m.SI; const SA = m.MSA || m.SA; const SC = m.SC;
...
// Keys: "EQ1EQ2EQ3EQ4EQ5" → base score
// Source: FIRST.org CVSS v4.0 specification lookup matrix
...
/**
 * Compute the CVSS v4.0 score from a FIRST.org vector string.
 * Throws a descriptive Error on invalid or incomplete input.
 */
