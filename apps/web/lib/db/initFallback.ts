...
export async function initInMemoryFallbackDb() {
  ...
  await seedFallbackData(memPool);
...
async function seedFallbackData(executor: any) {
  ...
  for (const c of compData) {
    await executor.query(`INSERT INTO components (id, name, product_id, description) VALUES ($1, $2, $3, $4)`, [
      c.id,
      c.name,
      c.prod,
      c.desc,
  ...
  for (const g of groupsData) {
    ...
    await executor.query(`INSERT INTO groups (id, name, description) VALUES ($1, $2, $3)`, [gid, g, `${g} group`]);
    groupIds[g] = gid;
  }

  // User Group Map
  await executor.query(`INSERT INTO user_group_map (user_id, group_id) VALUES ($1, $2)`, [
    userIds['carol_sec'],
  ...
  for (const b of bugDefs) {
    ...
    if (isSecurity) {
      await executor.query(`INSERT INTO bug_group_map (bug_id, group_id) VALUES ($1, $2)`, [
        bugId,
        groupIds['security-team'],
      ]);
    }
